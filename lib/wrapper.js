...
function transactionWrapper(func, name) {
    return function () {
        ...
        LOG.debug("New transaction: %s, tid: %s", name, transaction._id);
...
function callWrapper(func, name, tr_param) {
    return function () {
        var transaction = tr_param;
        if (!transaction)  {
            transaction = COLLECTOR.getTransaction();
            if (!transaction)
                return func.apply(this, arguments);
            LOG.debug("Wrapped call got tid:", transaction._id);
        }
        LOG.debug("Wrapped call: %s, tid:", name, transaction._id);
...
function callbackWrapper(callback, baseName, transaction, trace) {
    return function() {
        ...
        LOG.debug("Callback call: %s, tid: %s", name, transaction._id);
...
function callWithCallbackWrapper(func, name) {
    return function () {
        ...
        LOG.debug("Wrapped call: %s, tid: %s", name, transaction._id);
        var ret, trace, error;
